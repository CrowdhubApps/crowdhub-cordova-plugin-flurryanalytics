...
- (void)initialize:(CDVInvokedUrlCommand *)command {
    ...
    @try {
        ...
        FlurrySessionBuilder* sesh;
        if (options != nil) {
            sesh = [[[FlurrySessionBuilder new] withLogLevel:FlurryLogLevelAll] withCrashReporting:true];
            
            if ([options valueForKey:@"version"] != nil) {
               [sesh withAppVersion:options[@"version"]];
            }
             
            if ([options valueForKey:@"continueSessionSeconds"] != nil) {
               [sesh withSessionContinueSeconds:[options[@"continueSessionSeconds"] integerValue]];
           }
            ...
            if ([options valueForKey:@"logLevel"] != nil) {
                ...
                if ([@"VERBOSE" isEqualToString:level]) {
                    [sesh withLogLevel:FlurryLogLevelAll];
                } else if ([@"DEBUG" isEqualToString:level]) {
                    [sesh withLogLevel:FlurryLogLevelDebug];
                } else if ([@"INFO" isEqualToString:level]) {
                    [sesh withLogLevel:FlurryLogLevelDebug];
                } else if ([@"WARN" isEqualToString:level]) {
                    [sesh withLogLevel:FlurryLogLevelCriticalOnly];
            ...
            if ([options valueForKey:@"enableCrashReporting"] != nil) {
                [sesh withCrashReporting:[options[@"enableCrashReporting"] boolValue]];
            }
         

        }
    
        [Flurry startSession:appKey withSessionBuilder:sesh];
